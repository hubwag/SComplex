...
#define PROJECT_SOURCE_DIR "c:/users/hub/documents/simplex/"
#endif
...
template<typename SComplex>
boost::tuple<int, int, int, std::string>  CrHomSAKQ() {
	 boost::tuple<int, int, int, std::string> result;
	 using boost::tuples::get;
...
typedef CubSComplex<8>  SComplex;
  Stopwatch swTot;
  BOOST_TEST_MESSAGE(" --- Reading cubical cellular set ");
  ...
  BOOST_CHECK_EQUAL(std::string("1,2,1"), homSignCR().bettiVector());
