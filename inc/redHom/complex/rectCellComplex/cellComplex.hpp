...
cellComplex<T>::cellComplex( std::vector< cell<T>* > vect )
{
    ...
    for ( int i = max_dim ; i != 0 ; --i )
    {
        ...
        {
             for ( unsigned int t = 0 ; t != (*it)->coef.size() ; ++t )
             {
                  std::vector< std::pair<T,T> > firtsPoint;
                  std::vector< std::pair<T,T> > secondPoint;
                  for ( unsigned int o = 0 ; o != (*it)->coef.size() ; ++o )
                  {
                      if ( t != o )
                      {
                          firtsPoint.push_back( std::make_pair(((*it)->coef[o]).first , ((*it)->coef[o]).second) );
                          secondPoint.push_back( std::make_pair(((*it)->coef[o]).first , ((*it)->coef[o]).second) );
                      }
                      else
                      {
                          firtsPoint.push_back( std::make_pair(((*it)->coef[o]).first , ((*it)->coef[o]).first) );
                          secondPoint.push_back( std::make_pair(((*it)->coef[o]).second , ((*it)->coef[o]).second) );
                      }
                  }
                   cell<T>* firstCell = new cell<T>(firtsPoint);
                   cell<T>* secondCell = new cell<T>(secondPoint);
                   //deal with bd and cbd pointrs:
                   firstCell->coBound.push_back(*it);
                   secondCell->coBound.push_back(*it);
                   (*it)->bound.push_back(firstCell);
                   (*it)->bound.push_back(secondCell);
             }
        }

        //sort to remove repetitions:
        std::sort( this->elements[i-1].begin() , this->elements[i-1].end() , comp<T> );

        //now the repeted elements should be one after another in the vector. Remove them!
        typename std::set< cell<T>* >::iterator it, jt, buf ;
        it = this->elements[i-1].begin();
        while (it != this->elements[i-1].end() )
        {
            jt = it;
            ++jt;
            while ( (jt != this->elements[i-1].end())&&( **it == **jt ) )
            {
                 buf = jt;
                 ++jt;
                 //tie bd and cbd:
                 for ( typename std::list<cell<T>*>::iterator cbd = (*buf)->cbdBegin() ; cbd != (*buf)->cbdEnd() ; ++cbd )
                 {
                      (*it)->coBound.push_back( *cbd );
                 }
        ...
        //now remove redundancy from all cells bd and cbd lists:
        for ( typename std::set< cell<T>* >::iterator it = this->elements[i].begin() ; it != this->elements[i].end() ; ++it )
        {
              std::sort( (*it)->bound.begin() , (*it)->bound.end() , comp<T> );
              typename std::list< cell<T>* >::iterator it1, jt, buf ;
              it1 = (*it)->bound.begin();
              while (it1 != (*it)->bound.end() )
              {
                  jt = it1;
                  ++jt;
                  while ( (jt != (*it)->bound.end())&&( **it1 == **jt ) )
                  {
                       buf = jt;
                       ++jt;
                       (*it)->bound.erase( buf );
        ...
        {
            std::sort( (*it)->coBound.begin() , (*it)->coBound.end() , comp<T> );
            typename std::list< cell<T>* >::iterator it1, jt, buf ;
            it1 = (*it)->coBound.begin();
            while (it1 != (*it)->coBound.end() )
            {
                jt = it1;
                ++jt;
                while ( (jt != (*it)->coBound.end())&&( **it1 == **jt ) )
                {
                     buf = jt;
                     ++jt;
