...
template <typename T>
bool comp (cell<T>* i, cell<T>* j) { return (*i<*j); }
...
//********************************************************************************
       ...
       /*void sortGradation()
       {
           for ( unsigned int i = 0 ; i != this->elements.size() ; ++i )
           {
               //std::sort( this->elements[i].begin() , this->elements[i].end() , comp );
               std::sort( this->elements[i].begin() , this->elements[i].end() , comp<T> );
           }
       }*/
       ...
       dimIterator dimIteratorEnd(int i){return this->elements[i].end();};
        ...
        class constDimColoredCells : public dimColoredCells
        {
        public:
            explicit constDimColoredCells(cellComplex* cmplx , int dim, int color):dimColoredCells(cmplx,dim,color){}
            explicit constDimColoredCells():dimColoredCells(){}
        ...
        };//constDimColoredCells
        constDimColoredCells constDimColoredCellsBegin(int dim, int color)
        {
             if ( dim >= this->elements.size() )
             {
                  throw "Dim of iterator out of range for this complex.";
             }
             constDimColoredCells res(this,dim,color);
             res.pos = this->elements[dim].begin();
             while ( (res.pos != this->elements[dim].end()) && ((*(res.pos))->color() != color) )
             {
                   ++res;
        ...
        constDimColoredCells constDimColoredCellsEnd(int dim, int color)
        {
             if ( dim >= this->elements.size() )
             {
                  throw "Dim of iterator out of range for this complex.";
